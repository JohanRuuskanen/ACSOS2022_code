...
function update_params(params, p::Vector{T}, p_ind) where T <: Real
    Ψ, A, B, P, λ, M, K, p_prd = params

    P_new = Matrix{T}(P)
    P_new[p_ind] .= softmax(p)

    return (Ψ, A, B, P_new, λ, M, K, p_prd)
end

function update(params, w, p95_data, C, p_ind)

    # Caclulate the derivative of the cost function with respect to w
    dw = ForwardDiff.gradient(y -> costfunc(update_params(params, y, p_ind), p95_data, C), w)

    # Bound the step to the maximum norm
    p = softmax(w)
    p_next = softmax(w - alpha .* dw)
    if norm(p - p_next, 2) > p_step_size
        c = 0
        try
            c = fzero(c -> norm(p - softmax(w - c*alpha*dw), 2) - p_step_size, 0.0, 1.0)
        catch
    ...
    p95_pred = estimate_p95(update_params(params, w_new, p_ind))

    return w_new, dw, p95_pred
end
...
function findNextParam()
    ...
    p_new_prob = bound_probs(softmax(p_new))
    ...
    if !isfile(joinpath(logfolder, "data_$(tag).csv"))
        CSV.write(joinpath(logfolder, "data_$(tag).csv"), []; writeheader=true, header=[("p$i" for i in eachindex(p0))..., :ql, :ql_data, :p95, :p95_data, :cost, :cost_data, :violating, :p95_pred])
    end
    CSV.write(
        joinpath(logfolder, "data_$(tag).csv"), 
        (   (Symbol("p$i") => [p] for (i, p) in enumerate(softmax(p0)))..., 
